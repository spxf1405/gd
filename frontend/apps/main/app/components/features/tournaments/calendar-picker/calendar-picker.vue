...
const props = defineProps({
  ...
  toElement: { type: String, default: null },
});
...
// Đây là single source of truth để đóng/mở — tất cả đều đi qua đây
...
function selectDate(day) {
  ...
  emit("select", { date });
  // Đóng qua onOpenChange để Radix xử lý đúng flow
  ...
  <PopoverRoot :open="open" @update:open="onOpenChange">
    ...
    <PopoverTrigger as-child>
      <slot name="trigger">
        ...
        >
          <svg width="14" height="14" viewBox="0 0 16 16" fill="none" class="text-emerald-400 flex-shrink-0">
          ...
          <span :class="['text-[13px] tracking-[0.01em] transition-colors', modelValue ? 'text-white' : 'text-white/70']">
            {{ label }}
          </span>
          <button
            v-if="modelValue"
    ...
    <PopoverPortal>
      <PopoverContent
        ...
        :side-offset="8"
        align="start"
        class="cal-pop-in ag-custom-component-popup z-[999] w-[280px] rounded-[16px] p-4 bg-[#1a1d2e] border border-white/[0.08] shadow-[0_24px_60px_rgba(0,0,0,0.65),0_0_0_1px_rgba(255,255,255,0.04)] outline-none"
      >
        <!-- ── Header ── -->
        <div class="flex items-center justify-between mb-3">
          <template v-if="pickerMode === 'day'">
            ...
            >
              <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
                <path d="M4.5 3L7.5 6l-3 3" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" />
              </svg>
            </button>
          </template>
          <template v-else>
            <div class="flex items-center justify-between w-full">
              ...
              <span class="text-[11px] font-bold tracking-[0.12em] uppercase text-white/70">
        ...
        <!-- ── Month Picker ── -->
        <div v-if="pickerMode === 'month'" class="cal-pop-in grid grid-cols-3 gap-1.5">
          <button
            v-for="(m, i) in MONTHS"
            :key="m"
            @click="selectMonth(i)"
        ...
        <!-- ── Footer ── -->
        <div class="flex justify-between mt-3 pt-2.5 border-t border-white/[0.06]">
          <button
            @click="goToday"
