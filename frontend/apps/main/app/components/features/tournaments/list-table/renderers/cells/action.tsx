  ...
  setup(props) {
    ...
    const onSave = async () => {
      ...
      try {
        ...
        // TODO: gọi API ở đây với updatedData
        ...
        props.params.api.stopEditing(true); // revert nếu API lỗi
      } finally {
    ...
    const confirmDelete = async () => {
      try {
        await tournamentClient.deleteTournament({ id: data.id });
        open.value = false;
  ...
  render() {
    return (
      <div class="flex gap-2 py-2">
        ...
        <button
          class="px-3 py-1 bg-green-500 hover:bg-green-600 text-white rounded text-sm flex items-center gap-1 transition-all duration-150"
        ...
        {this.isEditing ? (
          <>
            {/* Save */}
            <button
              class="px-3 py-1 bg-blue-500 hover:bg-blue-600 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded text-sm flex items-center gap-1 transition-all duration-150"
              onClick={this.onSave}
              disabled={this.isSaving}
            >
              <Icon
                icon={this.isSaving ? "mdi:loading" : "mdi:save"}
                class={this.isSaving ? "animate-spin" : ""}
              />
              {this.isSaving ? "Đang lưu..." : "Save"}
            </button>

            {/* Huỷ edit */}
            <button
              class="px-3 py-1 bg-white/10 hover:bg-white/20 text-white/70 hover:text-white rounded text-sm flex items-center gap-1 transition-all duration-150"
          ...
          <>
            {/* Sửa */}
            <button
              class="px-3 py-1 bg-blue-500 hover:bg-blue-600 text-white rounded text-sm flex items-center gap-1 transition-all duration-150"
              onClick={this.onEdit}
            >
